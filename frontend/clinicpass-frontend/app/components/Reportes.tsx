...
export const Reportes: React.FC = () => {
  ...
  const estadisticasPorEstado = [
    { estado: 'Completados', cantidad: 1456, porcentaje: 72, color: 'bg-green-500' },
    { estado: 'Confirmados', cantidad: 345, porcentaje: 17, color: 'bg-blue-500' },
    { estado: 'Programados', cantidad: 178, porcentaje: 9, color: 'bg-yellow-500' },
    { estado: 'Cancelados', cantidad: 41, porcentaje: 2, color: 'bg-red-500' },
  ];

  const profesionalesMasActivos = [
    { nombre: 'Dra. María González', turnos: 487, fichas: 456, especialidad: 'Cardiología' },
    { nombre: 'Dr. Juan Pérez', turnos: 398, fichas: 378, especialidad: 'Pediatría' },
    { nombre: 'Dra. Laura Sánchez', turnos: 356, fichas: 334, especialidad: 'Traumatología' },
    { nombre: 'Dra. Carmen Díaz', turnos: 289, fichas: 267, especialidad: 'Dermatología' },
    { nombre: 'Dr. Roberto García', turnos: 234, fichas: 198, especialidad: 'Clínica Médica' },
  ];

  const maxTurnos = Math.max(...datosGenerales.map(d => d.turnos));
  const totalTurnos = datosGenerales.reduce((sum, d) => sum + d.turnos, 0);
  const totalPacientes = datosGenerales.reduce((sum, d) => sum + d.pacientes, 0);
        ...
        <section className="pdf-section">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <StatCard
              ...
              title="Total Turnos"
              value={totalTurnos.toLocaleString()}
            ...
            <StatCard
              ...
              value={totalFichas.toLocaleString()}
              subtitle={`${Math.round((totalFichas / totalTurnos) * 100)}% de completitud`}
              color="bg-purple-500"
            />
            <StatCard
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="relative h-64">
                <svg className="w-full h-full" viewBox="0 0 600 250">
                  {/* Líneas de guía */}
                  <line x1="40" y1="10" x2="40" y2="210" stroke="#e5e7eb" strokeWidth="1"/>
                  <line x1="40" y1="210" x2="590" y2="210" stroke="#e5e7eb" strokeWidth="1"/>
                  
                  {/* Líneas horizontales de referencia */}
                  {[0, 50, 100, 150, 200].map((y, i) => (
                    <g key={i}>
                      <line x1="40" y1={10 + y} x2="590" y2={10 + y} stroke="#f3f4f6" strokeWidth="1"/>
                      <text x="10" y={15 + y} className="text-gray-500" fontSize="10">
                        {250 - (i * 50)}
                      </text>
                    </g>
                  ))}
                  ...
                  <polyline
                    ...
                    strokeDasharray="5,5"
                    points={datosGenerales.map((d, i) => 
                      `${80 + (i * 43)},${210 - (d.pacientes / maxTurnos * 180)}`
                    ).join(' ')}
                  />
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="flex items-center justify-center mb-6">
                ...
                <svg className="w-48 h-48" viewBox="0 0 200 200">
                  ...
                  <circle cx="100" cy="100" r="80" fill="none" stroke="#6366f1" strokeWidth="40" strokeDasharray="175 327" transform="rotate(-90 100 100)" />
                  <circle cx="100" cy="100" r="80" fill="none" stroke="#3b82f6" strokeWidth="40" strokeDasharray="125 377" strokeDashoffset="-175" transform="rotate(-90 100 100)" />
                  <circle cx="100" cy="100" r="80" fill="none" stroke="#10b981" strokeWidth="40" strokeDasharray="100 402" strokeDashoffset="-300" transform="rotate(-90 100 100)" />
                  <circle cx="100" cy="100" r="80" fill="none" stroke="#f59e0b" strokeWidth="40" strokeDasharray="85 417" strokeDashoffset="-400" transform="rotate(-90 100 100)" />
                  <circle cx="100" cy="100" r="80" fill="none" stroke="#ef4444" strokeWidth="40" strokeDasharray="17 485" strokeDashoffset="-485" transform="rotate(-90 100 100)" />
                  
                  <text x="100" y="95" textAnchor="middle" className="text-gray-900" fontSize="24" fontWeight="bold">
                    {estadisticasPorEspecialidad.reduce((sum, e) => sum + e.turnos, 0)}
                  </text>
                  <text x="100" y="115" textAnchor="middle" className="text-gray-600" fontSize="12">
                    Turnos totales
                  </text>
                </svg>
              </div>
              ...
              <div className="space-y-3">
                {estadisticasPorEspecialidad.map((esp, index) => {
                  const colors = ['bg-indigo-600', 'bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-red-500'];
                  return (
                    <div key={esp.especialidad} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`w-3 h-3 ${colors[index]} rounded-full`}></div>
                        <span className="text-gray-700">{esp.especialidad}</span>
                      </div>
                      <span className="text-gray-900">{esp.turnos} ({esp.porcentaje}%)</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {estadisticasPorEstado.map((estado) => (
                <div key={estado.estado} className="text-center">
                  <div className={`${estado.color} h-24 rounded-lg mb-3 flex items-center justify-center`}>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-gray-900">Profesional</th>
                    <th className="px-6 py-3 text-left text-gray-900">Especialidad</th>
                    <th className="px-6 py-3 text-center text-gray-900">Turnos</th>
                    <th className="px-6 py-3 text-center text-gray-900">Fichas</th>
                    <th className="px-6 py-3 text-center text-gray-900">Completitud</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {profesionalesMasActivos.map((prof, index) => {
                    const completitud = Math.round((prof.fichas / prof.turnos) * 100);
                    return (
                      <tr key={prof.nombre} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-gray-700">{prof.especialidad}</td>
                        <td className="px-6 py-4 text-center text-gray-900">{prof.turnos}</td>
                        <td className="px-6 py-4 text-center text-gray-900">{prof.fichas}</td>
                        <td className="px-6 py-4 text-center">
                          <span className={`px-3 py-1 rounded-full ${
                            completitud >= 90 ? 'bg-green-100 text-green-700' :
                            completitud >= 75 ? 'bg-yellow-100 text-yellow-700' :
