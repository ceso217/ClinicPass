...
import { especialidadesClinica } from "../constants/especialidades";
...
export const Profesionales: React.FC = () => {
    ...
    try {
      if (selectedProfesional && selectedProfesional.id) {
        ...
        const updatedList = profesionales.map((p) =>
          p.id === selectedProfesional!.id
            ? ({ ...p, ...updatePayload } as Profesional)
            : p
        );
        ...
        if (espacioIndex === -1) {
          toast.error(
            "Por favor, ingrese el Nombre y el Apellido separados por un espacio (Ej: 'Juan Perez')."
          );
          return;
        }
      ...
      toast.error(
        `Ocurrió un error al guardar los datos. Detalles: ${
  ...
  const handleToggleActivo = async (profesional: Profesional) => {
    ...
    try {
      ...
      await updateProfesionalActivo(profesional.id as string);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {filteredProfesionales.length} profesional
              {filteredProfesionales.length !== 1 ? "es" : ""} •{" "}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            title={
                              profesional.activo ? "Dar de baja" : "Dar de alta"
                            }
                          >
                            {profesional.activo ? (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        rol: e.target.value as RoleType,
                      })
                    }
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                  >
                  ...
                  <>
                    {selectedProfesional && ( // Mensaje opcional para el modo Edición
                      <div className="md:col-span-2 text-sm text-gray-600 mt-2">
                        Deja los campos vacíos si no deseas cambiar la
                        contraseña.
                      </div>
                    )}
