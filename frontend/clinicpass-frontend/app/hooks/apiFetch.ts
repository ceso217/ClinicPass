...
export async function apiFetch(endpoint: string, options: RequestInit = {}) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 409) {
      errorMessage =
        "Los datos ingresados son invalidos, ya existe un usuario registrado con ese DNI o Email.";
    }

    try {
      // Intentamos leer el body como JSON usando el CLON.
      const errorBody = await clonedRes.json(); // Si la API devuelve un mensaje detallado (ej. "El DNI ya existe")
